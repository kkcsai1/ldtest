...
angular.module('angularMultiSlider', [])
    ...
    return {
      ...
      link: function(scope, element) {
        ...
        angular.forEach(scope.sliders, function(slider, key){
            var colorKey = slider.color ? '<span style="border-top-color:' + slider.color + ';"></span> ' : '';
  ...
  .directive('multiSlider',['$compile', '$filter' ,function($compile, $filter) {
    ...
    return {
      ...
      template :
      '<div class="bar row"><div class="col-md-2"></div><div class="col-md-2"></div><div class="col-md-2"></div><div class="col-md-2"></div><div class="col-md-2"></div></div>',
      ...
      link : function(scope, element, attrs, ngModel) {
        ...
        angular.forEach(scope.sliders, function (slider, key) {
            ...
            arrow_id = slider.title ? 'slider-'+slider.title : '';

            sliderStr += '<div class="handle '+arrow_id+'"><div class="shadow-hover"></div></div><div class="bubble">{{ sliders[' + key.toString() + '].title }}{{ sliders[' + key.toString() + '].value ' + filterExpression + ' }}</div>';
        ...
        var updateDOM = function () {
          ...
          var setHandles = function () {
            ...
            angular.forEach(scope.sliders, function(slider,key){
              if (slider.color) {
                  handles[key].css({ 'border-top-color': slider.color });
              }
          ...
          if (!bindingsSet) {
            ...
            setTimeout( function() {
              ...
              handleTop = handleTop === undefined ? (handles.length > 0 ? handles[0][0].offsetTop :undefined): handleTop;
              handleHeight = handleHeight === undefined ? (handles.length > 0 ? handles[0][0].offsetHeight :undefined) : handleHeight;
              bubbleTop = bubbleTop === undefined ? (handles.length > 0 ? bubbles[0][0].offsetTop :undefined) : bubbleTop;
              bubbleHeight = bubbleHeight === undefined ? (handles.length > 0 ? bubbles[0][0].offsetHeight + 7 : undefined) : bubbleHeight; //add 7px bottom margin to the bubble offset for handle
              if (angular.isUndefined(handleTop) || angular.isUndefined(handleHeight) || angular.isUndefined(bubbleTop) || angular.isUndefined(bubbleHeight))
                  return;
              resetBubbles();
            }, 10);
          }
